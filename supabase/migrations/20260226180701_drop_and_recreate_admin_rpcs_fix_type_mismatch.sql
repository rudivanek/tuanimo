/*
  # Drop and recreate admin RPC functions to fix type mismatch

  ## Problem
  Error 42804: auth.users.email is character varying but functions declare
  user_label as text. CREATE OR REPLACE cannot resolve this — must DROP first.

  ## Fix
  - DROP both functions
  - Recreate with explicit ::text casts on all returned string columns
*/

DROP FUNCTION IF EXISTS admin_token_usage_report(date, date, uuid);
DROP FUNCTION IF EXISTS admin_list_users_with_usage();
